...
# Pane 0: server
tmux send-keys -t "$SESSION_NAME":0.0 "$COMMON_PREFIX /opt/venv/bin/python3 scripts/realworld/http_internvla_server_debug.py --device $DEVICE --model_path $MODEL_PATH --calib $CALIB_PATH 2>&1 | tee $RUN_DIR/server_stdout.log" C-m
...
tmux send-keys -t "$SESSION_NAME":0.2 "$COMMON_PREFIX sleep $SERVER_WARMUP_SEC; python3.12 scripts/realworld/http_internvla_client_debug.py --calib $CALIB_PATH 2>&1 | tee $RUN_DIR/client.log" C-m
