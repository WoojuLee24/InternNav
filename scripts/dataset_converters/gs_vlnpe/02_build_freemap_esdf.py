...
def render_states(occupancy, origin, obstacle_ref, esdf_ref, cam_xyz, args, out_dir: Path):
    ...
    return [
        ...
        (f'④ navigable (r_b={args.r_b} truncate) + GT 궤적', save_jpg(_upscale(img_nav), out_dir / 'navigable.jpg')),
    ]
...
def main() -> int:
    ...
    sources = {'obj': lambda: load_scene_mesh(args.mesh_root, args.scene),
               'usd': lambda: load_scene_usd(scene_meta['usd_path'])}
    primary = 'obj' if args.geometry in ('obj', 'both') else 'usd'
    mesh = sources[primary]()
    occupancy, origin = voxelize_surface(mesh, args.cell_m, args.surface_samples,
    ...
    meta = {
        ...
        'ref_episode': args.ref_episode, 'ref_h_b': ref_h_b,
    ...
    body = (f'<h3>맵 단계별 (전부 같은 grid, cell={args.cell_m} m)</h3>'
            ...
            'obstacle이다(로봇 키 밴드). ④의 노란 점이 GT 궤적 — 초록(navigable) 안에 있어야 한다.</p>'
    ...
    print(f'  => {"PASS" if passed else "FAIL"} (clearance={gt_clearance["passed"]} sanity={sanity["passed"]})')
