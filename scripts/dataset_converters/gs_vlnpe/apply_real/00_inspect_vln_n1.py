...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--usd_path', default=None,
                        help='새 씬 USD/USDZ 자산. 지정하면 자산/좌표계/mesh 규약을 검사해 target_schema를 만든다')
    parser.add_argument('--floor_z', type=float, default=-0.05, help='새 씬에서 사용할 층의 바닥 높이(m)')
    return parser


def inspect_new_scene(args) -> int:
    """새 USDZ의 공간 GT와 파이프라인 규약을 검사한다. 자산은 수정하지 않는다."""
    ...
    try:
        ...
        bounds = np.asarray([aligned.GetMin(), aligned.GetMax()], dtype=np.float64)
        up_axis = str(UsdGeom.GetStageUpAxis(stage))
        meters_per_unit = float(UsdGeom.GetStageMetersPerUnit(stage))
        mesh_prims = [p for p in stage.Traverse() if p.IsA(UsdGeom.Mesh)]

        extents = bounds[1] - bounds[0]
        floor_delta = np.abs(np.asarray(mesh.triangles_center)[:, 2] - args.floor_z)
        floor_candidates = int(np.count_nonzero(
            (np.asarray(mesh.face_normals)[:, 2] >= np.cos(np.deg2rad(15.0))) & (floor_delta <= 0.45)))
        checks = {
            'usd_readable': True,
            'up_axis_z': up_axis.upper().endswith('Z'),
            'meters_per_unit_1': bool(np.isclose(meters_per_unit, 1.0)),
            'finite_bounds': bool(np.isfinite(bounds).all() and np.all(extents > 0)),
            'mesh_prims_present': bool(mesh_prims),
            'middle_floor_faces_present': floor_candidates > 0,
        }
        passed = all(checks.values())
        schema = {
            'schema_version': 'noeun_station_usdz_v1',
            'source': {'type': 'usdz', 'path': str(usd_path), 'scene': args.scene},
            'world_frame': {'up_axis': up_axis, 'meters_per_unit': meters_per_unit,
                            'pose_convention': 'cam2world_gl'},
            'geometry': {'mesh_prim_count': len(mesh_prims), 'triangle_count': int(len(mesh.faces)),
                         'bounds_min': bounds[0].tolist(), 'bounds_max': bounds[1].tolist(),
                         'floor_z': args.floor_z, 'middle_floor_candidate_faces': floor_candidates},
            'generated_gt_contract': {
                'path_stage': '03_sample_gt_paths.py',
                'observation_stage': '04_render_obs_isaac.py',
                'streams': ['rgb_uint8', 'depth_uint16', 'camera_extrinsic'],
            },
        ...
        xy_min, xy_max = vertices[:, :2].min(axis=0), vertices[:, :2].max(axis=0)
        size = 900
        span = np.maximum(xy_max - xy_min, 1e-6)
        scale = min((size - 40) / span[0], (size - 40) / span[1])
        canvas = np.full((size, size, 3), 38, np.uint8)
        floor_mask = ((np.asarray(mesh.face_normals)[:, 2] >= np.cos(np.deg2rad(15.0)))
                      & (floor_delta <= 0.45))
        pts = np.asarray(mesh.triangles[floor_mask]).reshape(-1, 3)
        if len(pts):
            px = ((pts[:, 0] - xy_min[0]) * scale + 20).astype(int)
            py = (size - 20 - (pts[:, 1] - xy_min[1]) * scale).astype(int)
            canvas[np.clip(py, 0, size - 1), np.clip(px, 0, size - 1)] = (90, 190, 255)
        floorplan_path = save_jpg(canvas, log_dir / 'middle_floor_candidates.jpg')
        rows = ''.join(
            f'<tr><td>{name}</td><td><span class="pill {"good" if ok else "bad"}">'
            f'{"PASS" if ok else "FAIL"}</span></td></tr>' for name, ok in checks.items())
        summary = (f'<div class="stat-row"><div class="stat"><b>판정</b>'
                   f'<span class="pill {"good" if passed else "bad"}">{"PASS" if passed else "FAIL"}</span></div>'
                   f'<div class="stat"><b>mesh prims / triangles</b><span class="pill">{len(mesh_prims)} / {len(mesh.faces)}</span></div>'
                   f'<div class="stat"><b>중간층 후보 면</b><span class="pill">{floor_candidates}</span></div></div>'
                   f'<table><tr><th>검사</th><th>결과</th></tr>{rows}</table>')
        body = (blink_widget_html('scene_asset', [('중간층 상향 수평면 후보', floorplan_path)],
                                  title='노은역 USDZ 공간 GT 진단')
                + '<h3>확정한 생성 GT 규약</h3>'
                + '<p>03이 경로 GT를 만들고, 04가 그 경로에서 RGB·uint16 depth·camera pose GT를 생성한다.</p>'
                + f'<pre>{json.dumps(schema, indent=2, ensure_ascii=False)}</pre>')
