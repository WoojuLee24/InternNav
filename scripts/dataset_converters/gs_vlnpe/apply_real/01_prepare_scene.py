...
from esdf_utils import load_scene_usd  # noqa: E402
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--usd_path', default=None,
                        help='GT가 없는 새 USD/USDZ 씬. 지정하면 기존 Matterport 검증 대신 logical level 메타를 만든다')
    parser.add_argument('--floor_z', type=float, default=-0.05, help='새 씬에서 선택할 logical level 바닥 높이(m)')
    parser.add_argument('--floor_band_m', type=float, default=0.45, help='바닥 면 선택 시 floor_z 주변 허용 폭(m)')
    parser.add_argument('--horizontal_angle_deg', type=float, default=15.0, help='바닥 후보 면의 최대 기울기(deg)')
    parser.add_argument('--headroom_m', type=float, default=1.60, help='logical level ROI의 floor_z 위 높이(m)')
    parser.add_argument('--xy_padding_m', type=float, default=1.0, help='검출된 바닥 XY bounds 패딩(m)')
    return parser


def prepare_new_usdz_scene(args) -> int:
    """USDZ 공간 GT에서 선택한 층의 ROI를 기록한다. 원본 자산은 수정하지 않는다."""
    ...
    try:
        ...
        mesh = load_scene_usd(str(usd_path))
        ...
        source_bounds = np.asarray([aligned.GetMin(), aligned.GetMax()], dtype=np.float64)

        normals = np.asarray(mesh.face_normals, dtype=np.float64)
        centers = np.asarray(mesh.triangles_center, dtype=np.float64)
        areas = np.asarray(mesh.area_faces, dtype=np.float64)
        min_up = np.cos(np.deg2rad(args.horizontal_angle_deg))
        floor_faces = ((normals[:, 2] >= min_up)
                       & (np.abs(centers[:, 2] - args.floor_z) <= args.floor_band_m))
        if not floor_faces.any():
            raise ValueError(f'no upward floor faces near z={args.floor_z}')

        floor_vertices = np.asarray(mesh.triangles[floor_faces], dtype=np.float64).reshape(-1, 3)
        xy_min, xy_max = floor_vertices[:, :2].min(axis=0), floor_vertices[:, :2].max(axis=0)
        bounds_min = np.array([xy_min[0] - args.xy_padding_m, xy_min[1] - args.xy_padding_m,
                               args.floor_z - 0.20])
        bounds_max = np.array([xy_max[0] + args.xy_padding_m, xy_max[1] + args.xy_padding_m,
                               args.floor_z + args.headroom_m])
        up_axis = str(UsdGeom.GetStageUpAxis(stage))
        meters_per_unit = float(UsdGeom.GetStageMetersPerUnit(stage))
        roi_inside = bool(np.all(bounds_min >= source_bounds[0]) and np.all(bounds_max <= source_bounds[1]))
        passed = bool(up_axis.upper().endswith('Z') and np.isclose(meters_per_unit, 1.0)
                      and np.all(bounds_max > bounds_min) and roi_inside)

        meta = {
            'scene_id': args.scene,
            'source_scene_id': Path(args.usd_path).stem.replace('_collision', ''),
            'scene_type': 'new_scene', 'logical_level': 'mid',
            'mesh_path': None, 'usd_path': str(usd_path), 'usd_docker_path': str(usd_path),
            'up_axis': 'Z', 'scale': 1.0,
            'usd_stage': {'up_axis': up_axis, 'meters_per_unit': meters_per_unit},
            'floor_z': args.floor_z,
            'floor_selection': {
                'method': 'up_facing_horizontal_surface_area',
                'probe_band_m': args.floor_band_m,
                'horizontal_angle_deg': args.horizontal_angle_deg,
                'selected_face_count': int(floor_faces.sum()),
                'selected_surface_area_m2': float(areas[floor_faces].sum()),
            },
            'source_bounds_min': source_bounds[0].tolist(), 'source_bounds_max': source_bounds[1].tolist(),
            'bounds_min': bounds_min.tolist(), 'bounds_max': bounds_max.tolist(),
            'floor_xy_bounds': {'min': xy_min.tolist(), 'max': xy_max.tolist()},
            'pose_convention': 'cam2world_gl', 'gt_robot_params': None,
            'gt_camera_z': None, 'gt_camera_height_m': None,
            'frame_alignment': None, 'gt_extent_check': None,
            'bounds_check': {'usd_found': True, 'logical_roi_inside_source': roi_inside},
            'passed': passed,
            'note': 'USDZ 공간 GT에서 선택한 logical level; source USDZ is not modified.',
        }
        ...
        scale = min((size - 40) / span[0], (size - 40) / span[1])
        floorplan = np.full((size, size, 3), 38, np.uint8)
        px = ((floor_vertices[:, 0] - xy_min[0]) * scale + 20).astype(int)
        py = (size - 20 - (floor_vertices[:, 1] - xy_min[1]) * scale).astype(int)
        floorplan[np.clip(py, 0, size - 1), np.clip(px, 0, size - 1)] = (90, 190, 255)
        floorplan_path = save_jpg(floorplan, log_dir / 'middle_floor_roi.jpg')
        status = 'good' if passed else 'bad'
        summary = (f'<div class="stat-row"><div class="stat"><b>판정</b>'
                   f'<span class="pill {status}">{"PASS" if passed else "FAIL"}</span></div>'
                   f'<div class="stat"><b>floor_z</b><span class="pill">{args.floor_z:+.3f} m</span></div>'
                   f'<div class="stat"><b>선택 면적</b><span class="pill">{meta["floor_selection"]["selected_surface_area_m2"]:.1f} m²</span></div></div>')
        body = (blink_widget_html('middle_floor', [('중간층 ROI 상향 수평면', floorplan_path)],
                                  title='노은역 중간층 공간 GT')
                + '<h3>USDZ logical level</h3><p>원본 USDZ는 수정하지 않았다. '
                '상향 수평면과 지정 floor_z로 중간층 ROI를 선택했으며 이후 02~04는 이 scene_meta를 사용한다.</p>'
                f'<pre>{json.dumps(meta, indent=2, ensure_ascii=False)}</pre>')
        report = save_gallery(log_dir, 'report.html', f'{SCRIPT_NAME} — {args.scene}', summary, body)
        print(f'  scene_meta -> {meta_path}\n  report html -> {report}')
        print(f'  floor_z={args.floor_z:+.3f}, area={meta["floor_selection"]["selected_surface_area_m2"]:.1f} m2')
...
def main() -> int:
    ...
    if args.usd_path:
        return prepare_new_usdz_scene(args)
