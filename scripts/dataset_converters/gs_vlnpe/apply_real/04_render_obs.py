...
from geometry_utils import (  # noqa: E402
    ...
    load_depth_frame_m,
    load_rgb_frame,
    load_scene_mesh as load_matterport_scene_mesh,
    save_jpg,
...
NEW_SCENE_GEOMETRY_PATH = None
DEPTH_SCALE_OUT_M = DEPTH_SCALE_RAW_TO_M
...
def load_scene_model(mesh_root, scene: str):
    ...
    if NEW_SCENE_GEOMETRY_PATH is not None:
        return o3d.io.read_triangle_model(str(NEW_SCENE_GEOMETRY_PATH))
    return o3d.io.read_triangle_model(str(find_scene_mesh(Path(mesh_root), scene)))


def load_scene_mesh(mesh_root, scene: str):
    if NEW_SCENE_GEOMETRY_PATH is not None:
        import trimesh
        return trimesh.load(str(NEW_SCENE_GEOMETRY_PATH), process=False, force='mesh')
    return load_matterport_scene_mesh(mesh_root, scene)


def prepare_usdz_open3d_geometry(usd_path: str) -> Path:
    """USDZ에 포함된 삼각 mesh를 임시 추출해 Open3D 입력으로 사용한다."""
    tmp_dir = Path(tempfile.mkdtemp(prefix='internnav_open3d_'))
    with zipfile.ZipFile(usd_path) as archive:
        mesh_member = next((n for n in archive.namelist() if Path(n).name == 'mesh.ply'), None)
        if mesh_member is None:
            raise FileNotFoundError(f'mesh.ply not found in {usd_path}')
        archive.extract(mesh_member, tmp_dir)
    return tmp_dir / mesh_member
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--usd_path', default=None, help='새 씬 USDZ. 내부 mesh.ply를 Open3D로 렌더링한다')
    parser.add_argument('--camera', default='dataset', choices=['dataset', 'd455_nominal'])
...
def depth_m_to_raw16(depth_m: np.ndarray, valid_mask: np.ndarray) -> np.ndarray:
    ...
    scaled = np.clip(np.round(depth_m / DEPTH_SCALE_OUT_M), 1, 65534)
...
def run_generate(args) -> dict:
    ...
    if args.camera == 'd455_nominal':
        k = camera_profiles.get(args.camera).k
    else:
    ...
    stats = {
        ...
        'rgb_mean': float(np.mean([r['rgb_mean'] for r in results])) if results else float('nan'),
        'rgb_std': float(np.mean([r['rgb_std'] for r in results])) if results else float('nan'),
    }
    stats['step_ok'] = bool(stats['step_bad_total'] == 0)
    stats['anchor_ok'] = bool(stats['anchor_worst_m'] <= GEN_MESH_ANCHOR_TOL_M)
    stats['rgb_ok'] = bool(5.0 < stats['rgb_mean'] < 250.0 and stats['rgb_std'] >= 10.0)
    stats['passed'] = bool(stats['step_ok'] and stats['anchor_ok'] and stats['rgb_ok']
                           and len(results) == len(episodes_data))
...
<div class="stat-row">
  ...
  <div class="stat"><b>RGB 정보량</b><span class="pill {"good" if stats["rgb_ok"] else "bad"}">
      mean {stats["rgb_mean"]:.1f} / std {stats["rgb_std"]:.1f}</span></div>
</div>
...
def main() -> int:
    ...
    global NEW_SCENE_GEOMETRY_PATH, DEPTH_CLIP_MIN_M, DEPTH_CLIP_MAX_M, DEPTH_SCALE_OUT_M
    if args.usd_path:
        NEW_SCENE_GEOMETRY_PATH = prepare_usdz_open3d_geometry(args.usd_path)
    if args.camera == 'd455_nominal':
        profile = camera_profiles.get(args.camera)
        DEPTH_CLIP_MIN_M, DEPTH_CLIP_MAX_M = profile.depth_min_m, profile.depth_max_m
        DEPTH_SCALE_OUT_M = profile.depth_scale_m
