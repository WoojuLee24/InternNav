#!/usr/bin/env python3
"""Parse Gate 3m (I-055 TR-EMA) results and print paper-ready table rows."""
import json, math, os
...
print("=== Gate 3m: Transition-Reset EMA (I-055) α Sweep ===\n")

print("--- BASELINE (TR-EMA=off, MH=15) ---")
for b in BAGS:
    ...
    if os.path.exists(path):
        ...
        nat = bg - aa - mh
        print(f"  bag={b}: skip={skip:.1f}%  bg={bg}  AA={aa}  MH={mh}  nat={nat}  ar={ar:.1f}%")
    else:
        print(f"  bag={b}: NOT FOUND")

print()
print(f"{'Alpha':>8} {'Skip%':>8} {'V_073623':>10} {'V_061841':>10} {'V_063047':>10} {'V_max':>8} {'Status':>8}")
print("-" * 75)
...
for alpha in ALPHAS:
    ...
    if not skip_vals:
        print(f"{alpha:>8.2f}   N/A (not run)")
        continue
    ...
    print(f"{alpha:>8.2f} {skip_mean:>7.1f}% {v073s:>10} {v061s:>10} {v063s:>10} {v_max:>8.4f} {status:>8}")
