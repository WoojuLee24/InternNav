...
run_name=temporal_modeling/num_history_16
output_dir=checkpoints/${run_name}
system1=nextdit_async
system2_ckpt=checkpoints/InternVLA-N1-System2

# [ablation] num_history: 16 (baseline: 8)
num_history=16
...
" 2>/dev/null)
    ...
    if [ -n "${best_ckpt}" ]; then
        ...
        wandb_run_id=$(ls -td "${output_dir}/wandb/run-"* 2>/dev/null | head -1 | xargs basename | sed 's/^run-[0-9]*_[0-9]*-//')
